%function b = ridge_modified(y,X,k,flag)
...
clear;
load('../../DerivedData/model_data/Subject_5/Electrode_11/Timepoint_1');
y=data(:,1);
X=data(:,2:end);
k=[0.5e-3,1e-3,1e-2,1e-1];
flag=0;
k=0.1;
tic;
b=ridge(y,X,k,flag);
...
%clearvars MX STDX X idx

% Compute the ridge coefficient estimates using the technique of
% adding pseudo observations having y=0 and 'X = k*I.
%pseudo = sqrt(k(1)) * eye(p);
%Zplus=zeros(n+p,p);
%Zplus(1:n,:)=Z;
%for i=1:p
%	Zplus(n+i,i)=sqrt(k(1));
%end
%Zplus(n+1:n+p,:)=sqrt(k(1)) * eye(p);
