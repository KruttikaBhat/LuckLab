function get_model_data_leftrightencode(output,e_list,subjects,scenes,timepoints,numpixels,gms_filename,parent_folder,locationindex)
...
for i=1:length(subjects)
	...
	for j=1:numpixels
		...
		for k=1:length(scenes)
			temp_output=[temp_output;repmat(locationindex(scenes(k))+1,sum(numtrials{subjects(i)}==scenes(k)),1)]; %non averaging, encode scenes
